...
from modules import seed, models, data_utils_2protein, visualizations, training_utils_filip
...
def main():
    ...
    h2 = 2
    dropout = 0.1
    trained_model = models.ExtendedFILIP(input_dim, embedding_dim, h1, h2, dropout, esm_model).to(device)
    ...
    visualizations.plot_embedding_cosine_similarities_filip(base_path, "Raw Embedding Cosine Similarities", train_loader, tokenizer, trained_model, device)
    ...
    with open(losses_save_path, 'w') as f:
        ...
        for epoch in range(num_epochs):
            print('new epoch')
            train_loss = training_utils_filip.train(trained_model, train_loader, optimizer, tokenizer, device)
            val_loss = training_utils_filip.evaluate(trained_model, val_loader, tokenizer, device)
    ...
    best_trained_model = models.ExtendedFILIP(input_dim, embedding_dim, h1, h2, dropout, esm_model).to(device)
    best_trained_model.load_state_dict(torch.load(model_save_path))
    test_loss = training_utils_filip.evaluate(best_trained_model, test_loader, tokenizer, device)
    print(f"Test Loss: {test_loss:.4f}") 

    visualizations.plot_loss_curves(base_path, train_losses, val_losses, batch_size, batch_size)

    visualizations.plot_embedding_cosine_similarities_filip(base_path, "Trained Embedding Cosine Similarities on Train Set", train_loader, tokenizer, best_trained_model, device)    
    visualizations.plot_embedding_cosine_similarities_filip(base_path, "Trained Embedding Cosine Similarities on Val Set", val_loader, tokenizer, best_trained_model, device)
